...
void chooseMode(){
  ...
  //boolean buttonState = false; //declare local boolean 
...
void live()
{
   ...
   //retrieve value from photocell and convert the value
   offsetFrequency = getPhotoFrequency();
   // output sound'
    //output sound and adjust noise (seen from example in class
    int theAdjuster = 8;

   //analog value of button keys
   int pinVal = analogRead(NOTE_IN_PIN);

   //evaluate value from pinval is higher than the noise
   if (pinVal > theAdjuster)
   //play tone
   tone(BUZZER_PIN, pinVal + offsetFrequency, 100);   //tone(pin, frequency, duration);

    //valu photo + note freq = output buzzer
   //duration == 100;
...
void recordWithDuration()
{
  ...
  if (countNotes < MAX_NOTES)
  {
    ...
    else if (testNote > theAdjuster && (timePassed > MAX_PRESS_TIME))
    {
      ...
      noTone(BUZZER_PIN);    //turn off any tone playing by using noTone function and input the pin to shut off
...
void startUpTimer()
{
 ...
 //assign the current time in milliseconds using millis function
 startTime = millis();

 //reset time passed to  allow proper calculation of time passing in user's experience
 timePassed = 0; 

 //button is active boolean must be true button is being pressed
...
void updateTimer()
{
 ...
 //IMPLEMENT
 //update timer to current time by subtracting starttime from it
...
void playCurrentNote()
{
    ...
    //to hold the tone of the notes for fluid waveforms use active frequency
...
void updateArraysWithNoteAndTimings()
{
 ...
 if(countNotes < MAX_NOTES) {
  ...
  //fill durations array
  durations[countNotes] = timePassed;

  //increment counter of ntoes
  countNotes++;
  ...
  //make button false when not pressed
  activeNoteButton = false;
  
  }

  /*
  notes [MAX_NOTES] = 0;    //array 1
  durations [MAX_NOTES] = 0;   //array 2
  countNotes = 0;   //counter
  activeNoteButton = false;      //boolean
  */
...
{
  //IMPLEMENT
  //get.getRunningAverage();   //do we need to get function get average of photocell frequency before returning its value???//
  //use integer variable for photocell value
  int photoVal = analogRead(PHOTO_PIN);

  //return the value properly scaled
...
{
 //IMPLEMENT
 //getPhotoFrequency() + avereage = thing ;
 ...
 for (int i = 0; 1 < RUNNING_SAMPLES; i++) {
 ...
 */
 ...
 //total sum of photocell input 
 int totalSumPhotocell = 0;

 //must loop 16x to calculate sum
 for(int i = 0; i < RUNNING_SAMPLES; i++) {
  //add sum
  totalSumPhotocell += runningAverageBuffer[i];
  
 }

 //return average value of photocell
 return totalSumPhotocell / RUNNING_SAMPLES;
...
void playWithDuration()
{
  ...
  //arrays duration
  //note play tone 
  //color led analog call colorLED function 
  //mode button

  //check the user has not exceeded the array's limit for the notes
  if(countNotes < MAX_NOTES) {

    //play sound
    tone(BUZZER_PIN, notes[countNotes], durations[countNotes]);

    //adjust LED intensity and scale note frequency 
    colorLED(map(notes[countNotes], 0, 1023, 0, 255));

    //add some delay to allow the circuit to process and then play the tone
...
/*
  durations [MAX_NOTES] = testNote;   
  tone(BUZZER_PIN, analogRead(offsetFrequency), 100);    //play the note using tone function
  colorLED(255);      //call the function colorLED with the argument as 255 for color
  digitalRead(BUTTON_MODE_PIN);   //value of mode button being pressed
  */
  /*    //count through array of duration while i is less than
    for(int i = 0; i < countNotes; i++) {
